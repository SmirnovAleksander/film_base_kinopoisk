...
def add_bookmark(film_id: int, user_id: int = Depends(get_current_user_id), conn = Depends(get_conn)):
    cur = conn.cursor()
    try:
...
def remove_bookmark(film_id: int, user_id: int = Depends(get_current_user_id), conn = Depends(get_conn)):
    cur = conn.cursor()
    try:
        cur.execute(
...
def check_bookmark_status(film_id: int, user_id: int = Depends(get_current_user_id), conn = Depends(get_conn)):
    cur = conn.cursor()
    try:
        cur.execute(
